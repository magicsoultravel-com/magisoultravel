...
/* Outer wrapper for all posts - match your other sections */
.section.all-posts {
...
<div id="admin-modal" class="modal-overlay">
    <div class="modal-content">
        ...
        <form action="<?= htmlspecialchars($_SERVER['PHP_SELF'] . '?' . http_build_query($_GET)) ?>" method="post">
            ...
            <p><label>Content:<br><textarea name="content" id="content-input" rows="10" required></textarea></label></p>
...
document.querySelectorAll('.edit-post-button').forEach(button => {
    button.addEventListener('click', function() {
        ...
        dateInput.value = `${postDate.getFullYear()}-${pad(postDate.getMonth()+1)}-${pad(postDate.getDate())}T${pad(postDate.getHours())}:${pad(postDate.getMinutes())}`;
        adminModal.style.display = 'flex';
        autoResizeTextarea(contentTextarea);
    });
});

document.querySelectorAll('.close, .cancel-modal').forEach(btn => btn.addEventListener('click', closeModal));
contentTextarea.addEventListener('input', function(){ autoResizeTextarea(this); });
</script>
<?php endif; ?>
...
document.querySelectorAll('.read-more-toggle').forEach(button => {
    button.addEventListener('click', function(e) {
        ...
        document.querySelectorAll('.post-item.expanded').forEach(otherPost => {
            if(otherPost!==section){otherPost.classList.remove('expanded'); const otherToggle=otherPost.querySelector('.read-more-toggle'); if(otherToggle){otherToggle.textContent='Read more...';}}
        });
        if(!isExpanded){section.classList.add('expanded'); this.textContent='Show less...';} 
        else {section.classList.remove('expanded'); this.textContent='Read more...';}
    });
});

// Show/hide "Read more" link if content overflows
function checkOverflow(){
    document.querySelectorAll('.post-summary').forEach(summaryDiv=>{
        ...
        const toggleLink = summaryDiv.closest('.post-item').querySelector('.read-more-toggle');
        if(isOverflowing){toggleLink.style.display='block';} else {toggleLink.style.display='none';}
    });
}
